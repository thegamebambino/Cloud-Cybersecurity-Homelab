...
resource "aws_instance" "windows-ad" {
  ...
  tags = { 
    Name = "Cybersecurity Homelab [Windows_AD]"
  }
...
# Output Windows AD IP Address.
output "instance_public_ip_win-ad" {
  value = "Windows Box IP Address: ${aws_instance.windows-ad.public_ip}"
}
...
# Output Windows 10 IP Address.
output "instance_public_ip_win-10" {
  value = "Windows Box IP Address: ${aws_instance.windows-10.public_ip}"
}
